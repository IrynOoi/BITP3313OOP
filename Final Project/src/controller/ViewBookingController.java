        ...
        try (Connection con = DatabaseConnection.doConnection())
        {
            ...
            try (PreparedStatement stmt = con.prepareStatement(query)) {
                ...
                if (rs.next()) {
                 ...
                 // Fetch airline name correctly
                    String airlineName = rs.getString("Airline_Name");

                    // Create an Airline object
                    Airline airline = new Airline(airlineName); // Assuming Airline has a constructor accepting name

                    // Construct the Booking object with an Airline object
                    booking = new Booking(
                        rs.getInt("BookingID"), 
                        rs.getString("Booking_Date"), 
                        customer, flight, seat, 
                        baggage, rs.getDouble("totalPrice"), 
                        airline  // ✅ Correct: Passing an Airline object
                    );
